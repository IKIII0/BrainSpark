...
const AdminDashboard = () => {
  ...
  const handleViewQuizzes = async (materiId) => {
    try {
      setQuizLoading(true);
      ...
      const quizData = await quizService.getQuizByMateriId(materiId);
      setQuizzes(quizData || []);
  ...
  const handleQuizChange = (e) => {
    ...
    if (name.startsWith('option')) {
      ...
      setNewQuiz(prev => ({
        ...prev,
        [name]: name === 'correct_answer' ? Number(value) : value
      }));
    }
  };
  ...
  const handleAddQuiz = async (e) => {
    ...
    try {
      ...
      // Reset form
      setNewQuiz({
        materi_id: newQuiz.materi_id, // Keep selected materi
        question: '',
        options: ['', '', '', ''],
        correct_answer: 0,
      });
  ...
  const handleDeleteQuiz = async (quizId) => {
    const result = await Swal.fire({
      ...
      text: 'Soal quiz ini akan dihapus secara permanen.',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Ya, hapus',
      cancelButtonText: 'Batal',
              ...
              <div className="p-6 border-b border-gray-200">
                <div className="flex justify-between items-center">
                  <h2 className="text-2xl font-bold text-gray-800">
                    Kelola Quiz - {materials.find(m => m.id === selectedMateri)?.title}
                  </h2>
                  <button
                    onClick={() => {
                      ...
                      >
                        {newQuiz.options.map((option, index) => (
                          <option key={index} value={index}>
                            {String.fromCharCode(65 + index)} - {option || '(kosong)'}
                          </option>
                        ))}
                      </select>
                    </div>
                ...
                <div>
                  ...
                  {quizLoading ? (
                    ...
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p>Belum ada soal untuk materi ini</p>
                    </div>
                  ) : (
                            ...
                            <div className="flex-1">
                              <div className="flex items-center mb-2">
                                ...
                                <span className="text-xs text-gray-500">
                                  Jawaban: {String.fromCharCode(65 + quiz.correct_answer)}
                                </span>
                              </div>
                              <p className="text-gray-800 font-medium mb-2">{quiz.question}</p>
                              <div className="grid grid-cols-2 gap-2 text-sm">
                                {Array.isArray(quiz.options) ? quiz.options.map((option, optIndex) => (
                                  <div key={optIndex} className={`p-2 rounded ${optIndex === quiz.correct_answer ? 'bg-green-50 text-green-800 font-medium' : 'bg-gray-50 text-gray-600'}`}>
                                    {String.fromCharCode(65 + optIndex)}. {option}
                                  </div>
                                )) : JSON.parse(quiz.options || '[]').map((option, optIndex) => (
                                  <div key={optIndex} className={`p-2 rounded ${optIndex === quiz.correct_answer ? 'bg-green-50 text-green-800 font-medium' : 'bg-gray-50 text-gray-600'}`}>
                                    {String.fromCharCode(65 + optIndex)}. {option}
                                  </div>
                                ))}
